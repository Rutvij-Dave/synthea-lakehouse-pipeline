...
@dp.table(name="claims_lakehouse.silver.silver_patient_fhir_r4")
def silver_patient_fhir_r4():
    return (spark.read.table("claims_lakehouse.bronze.bronze_fhir_r4_raw")
      ...
      .select(F.col("r.id").alias("patient_id"),F.col("r.gender").alias("gender"),
              F.to_date(F.col("r.birthDate")).alias("birth_date"),
              F.to_json(F.col("r.name")).alias("patient_name_json"),
      ...
      .filter(F.col("patient_id").isNotNull()).dropDuplicates(["patient_id"]))
