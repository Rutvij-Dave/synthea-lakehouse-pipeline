...
WITH line_agg AS (
    SELECT
        ...
        SUM(line_net_amount) AS line_net_amount_total,
...
SELECT
    ...
    l.line_count_from_detail,
    l.line_net_amount_total,
    l.avg_line_amount,
    l.max_line_amount,
    l.min_line_amount,

    CASE
        WHEN l.line_count_from_detail IS NULL
          OR c.claim_total_amount IS NULL
        THEN NULL
        ELSE c.claim_total_amount - l.line_net_amount_total
    END AS claim_vs_line_amount_difference,

    CASE
        WHEN c.claim_total_amount IS NULL
          OR c.claim_line_count IS NULL
          OR c.claim_line_count = 0
        THEN NULL
    ...
    CASE
        WHEN c.claim_total_amount IS NULL
          OR c.claim_total_amount < 0
        THEN NULL
        ELSE LOG1P(c.claim_total_amount)
    END AS claim_amount_log,

    CASE
        WHEN c.service_start_date IS NOT NULL
    ...
    CASE
        WHEN c.patient_id IS NULL OR TRIM(c.patient_id) = ''
        THEN NULL
        ELSE p.patient_claim_count
    END AS patient_claim_count,

    CASE
        WHEN c.patient_id IS NULL OR TRIM(c.patient_id) = ''
        THEN NULL
        ELSE p.patient_total_claim_amount
    END AS patient_total_claim_amount,

    CASE
        WHEN c.patient_id IS NULL OR TRIM(c.patient_id) = ''
        THEN NULL
        ELSE p.patient_avg_claim_amount
    END AS patient_avg_claim_amount,

    CASE
        WHEN c.patient_id IS NULL OR TRIM(c.patient_id) = ''
        THEN NULL
        ELSE p.patient_max_claim_amount
    END AS patient_max_claim_amount,

    CASE
        WHEN c.patient_id IS NULL OR TRIM(c.patient_id) = ''
        THEN NULL
        ELSE p.patient_avg_line_count
    END AS patient_avg_line_count,

    CASE
        WHEN c.provider_id IS NULL OR TRIM(c.provider_id) = ''
        THEN NULL
        ELSE pr.provider_claim_count
    END AS provider_claim_count,

    CASE
        WHEN c.provider_id IS NULL OR TRIM(c.provider_id) = ''
        THEN NULL
        ELSE pr.provider_unique_patient_count
    END AS provider_unique_patient_count,

    CASE
        WHEN c.provider_id IS NULL OR TRIM(c.provider_id) = ''
        THEN NULL
        ELSE pr.provider_total_claim_amount
    END AS provider_total_claim_amount,

    CASE
        WHEN c.provider_id IS NULL OR TRIM(c.provider_id) = ''
        THEN NULL
        ELSE pr.provider_avg_claim_amount
    END AS provider_avg_claim_amount,

    CASE
        WHEN c.provider_id IS NULL OR TRIM(c.provider_id) = ''
        THEN NULL
        ELSE pr.provider_max_claim_amount
    END AS provider_max_claim_amount,

    CASE
        WHEN c.provider_id IS NULL OR TRIM(c.provider_id) = ''
        THEN NULL
        ELSE pr.provider_avg_line_count
    END AS provider_avg_line_count,

    CASE
        WHEN c.patient_id IS NULL OR TRIM(c.patient_id) = ''
        THEN NULL
        WHEN p.patient_avg_claim_amount IS NULL
          OR p.patient_avg_claim_amount = 0
        THEN NULL
    ...
    CASE
        WHEN c.provider_id IS NULL OR TRIM(c.provider_id) = ''
        THEN NULL
        WHEN pr.provider_avg_claim_amount IS NULL
          OR pr.provider_avg_claim_amount = 0
        THEN NULL
        ELSE c.claim_total_amount / pr.provider_avg_claim_amount
    END AS claim_to_provider_avg_ratio,

    CASE
        WHEN c.claim_total_amount >= 1000 THEN 1
        ELSE 0
    END AS high_amount_flag,

    CASE
        WHEN c.claim_line_count > 1 THEN 1
        ELSE 0
    END AS multi_line_flag,

    CASE
        WHEN c.patient_id IS NULL OR TRIM(c.patient_id) = ''
        THEN 1
        ELSE 0
    END AS missing_patient_flag,

    CASE
        WHEN c.provider_id IS NULL OR TRIM(c.provider_id) = ''
        THEN 1
        ELSE 0
    END AS missing_provider_flag,
